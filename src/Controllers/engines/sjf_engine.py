"""Shortest Job First engine supporting preemptive and non-preemptive modes."""
...
class SJFEngine(BaseSchedulerEngine):
    ...
    def _compute_non_preemptive(self, processes: list[ProcessSpec]) -> ScheduleResult:
        ...
        while pending or ready:
            ...
            ready.sort(key=lambda s: (s.burst_time, s.arrival_time, s.spec.insertion_order))
            current = ready.pop(0)
    ...
    def _compute_srtf(self, processes: list[ProcessSpec]) -> ScheduleResult:
        timeline = []
        states = self.build_runtime_states(processes)
        pending = sorted(states, key=lambda s: (s.arrival_time, s.spec.insertion_order))
        ready = []
        ...
        while pending or ready:
            ...
            ready.sort(key=lambda s: (s.remaining_time, s.arrival_time, s.spec.insertion_order))
            current = ready[0]
            if current.first_start_time is None:
                current.first_start_time = time
            current.status = ProcessStatus.RUNNING
